# jsphh-plugins

Personal Claude Code plugin marketplace for academic research and teaching.

## Installation

Add this marketplace once on any device:

```bash
/plugin install github:jsphh7124/jsphh-plugins
```

## Plugins

| Plugin | Description | Skills |
|--------|-------------|--------|
| [jsphh-academic](./jsphh-academic) | Academic research toolkit | academic-editor, content-research-writer, literature-review, scientific-critical-thinking, nlm-skill, social-psych-activity |
